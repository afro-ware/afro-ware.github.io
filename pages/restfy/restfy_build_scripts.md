...
permalink: restfy_build_scripts.html
folder: mydoc
---
...
echo "Now run . restfy_2_multibuild_pdf.sh"
```
...
prince --javascript --input-list=../doc_outputs/mydoc/writers-pdf/prince-file-list.txt -o mydoc/files/restfy_writers_pdf.pdf;
echo "done"

# Doc Designers
echo "Building Mydoc Designers PDF ..."
prince --javascript --input-list=../doc_outputs/mydoc/designers-pdf/prince-file-list.txt -o mydoc/files/restfy_designers_pdf.pdf;
echo "done"

echo "All done building the PDFs!"
echo "Now build the web outputs: . restfy_3_multibuild_web.sh"
```
...
echo "Now push the builds to the server with . restfy_4_publish.sh"
```
...
scp -r -vrC ../restfy_outputs/doc-writers yourusername@yourdomain:/var/www/html/yourpublishingdirectory
...
Also, in case you haven't set up the SSH publishing without a password, see [Getting around the password prompts in SCP][restfy_no_password_prompts_scp]. Otherwise the script will stop and ping you to enter your password for each directory it transfers.
...
After you've configured all the scripts, you can run them all by running `. restfy_all.sh`. You might want to run this script at lunchtime, since it may take about 10 to 20 minutes to completely build the scripts. But note that since everything is now automated, you don't have to do anything at all after executing the script. After the script finishes, everything is published and in the right location.
